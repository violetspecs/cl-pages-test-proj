...
async function migrate(db, callback, config) {
  ...
  }).from(import_sql.sql.raw(migrationsTable)).orderBy(
  ...
  for (const migration of migrations) {
    if (!lastDbMigration || Number(lastDbMigration.created_at) < migration.folderMillis) {
      queriesToRun.push(
        ...migration.sql,
        `insert into ${import_sql.sql.identifier(migrationsTable)} (\`hash\`, \`created_at\`) values(${migration.hash}, ${migration.folderMillis})`
      );
    }
  }
